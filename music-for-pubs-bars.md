---
layout: venue
permalink: /music-for-pubs-bars/
slug: music-for-pubs-bars
label: Pubs & Bars
venue_name: pubs & bars
title: Music for Pubs & Bars Australia | Jukebox & Music | eJukebox
description: Background music and an interactive jukebox for Australian pubs & bars. Patrons request songs from their phone. OneMusic licence included. Free trial.
h1: Music for Pubs & Bars
intro: The right music keeps patrons in the room and ordering at the bar. eJukebox gives your pub or bar a professionally curated, fully licensed soundtrack your staff steer from a phone, and lets customers request the songs they want to hear, no DJ and no jukebox box bolted to the wall.
benefits:
  - icon: fa-beer-mug-empty
    title: Keep the bar buzzing
    text: Curated playlists tuned to the time of day, from quiet afternoon trade to a packed Friday night.
  - icon: fa-mobile-screen
    title: Song requests from any phone
    text: Patrons scan a QR code and request tracks. You stay in control with approvals, banned tracks and volume caps.
  - icon: fa-certificate
    title: Licence included
    text: Your OneMusic licence is bundled, so every track you play is legal, with no separate paperwork.
faqs:
  - q: Do I need a music licence to play music in my pub?
    a: Yes. Australian venues need a OneMusic Australia licence to play recorded music in public. With eJukebox that licence is included in your subscription, so you are covered automatically.
  - q: Can customers really request songs?
    a: Yes, interactive song requests are at the heart of eJukebox. Patrons request from their own phone and you keep full control over what actually plays.
---
